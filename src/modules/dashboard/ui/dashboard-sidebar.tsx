...
export const DashboardSidebar = () => {
  ...
  return (
    <aside className="w-64 h-screen bg-primary/5 border-r border-slate-200 dark:border-slate-800 flex flex-col">
      {/* Header */}
      <div className="h-16 px-4 flex items-center border-b border-slate-200 dark:border-slate-800">
        <Link href="/" className="flex items-center gap-2.5 min-w-0">
          ...
          <span className="text-2xl font-semibold text-slate-900 dark:text-white tracking-tight truncate">
            mentri.ai
          </span>
        </Link>
      </div>
          ...
          {firstSection.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  "flex items-center gap-3 px-2.5 h-9 rounded-md text-[14px] font-medium transition-colors",
                  isActive
    ...
    <Link
      ...
      href={item.href}
      className={cn(
        "flex items-center gap-3 px-3 h-10 rounded-lg text-[14px] font-medium transition-all duration-200",
        "border",
        isActive
